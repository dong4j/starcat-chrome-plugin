...
(async function () {
  ...
  const I18N = {
    en: {
      ...
      "hero.title": "Starcat Browser Plugin",
      ...
      "pairing.description": "Use the service URL and token shown in Starcat Settings > Browser Plugin.",
      ...
      "notice.body": "The extension stores only the service URL and token. Repository context and notes stay owned by Starcat.",
      ...
      "setup.step2": "Copy the service URL and bearer token into this page.",
      ...
      "troubleshooting.check3": "GitHub pages may need a hard refresh after changing the service URL or token.",
    ...
    zh: {
      ...
      "hero.title": "Starcat 浏览器插件",
      "hero.lead": "将 Starcat 上下文引入 GitHub 仓库页面，同时保持数据通过本地 Starcat 应用程序路由。",
      "pairing.eyebrow": "配对",
      "pairing.title": "本地服务",
      "pairing.description": "使用 Starcat 设置 > 浏览器插件中显示的服务地址和令牌。",
      ...
      "links.open": "打开",
      "notice.title": "局部优先边界",
      "notice.body": "该扩展程序仅存储服务地址和令牌。仓库上下文和备注仍归 Starcat 所有。",
      ...
      "troubleshooting.check3": "更改服务地址或令牌后，GitHub 页面可能需要强制刷新。",
  ...
  const storedPrefs = await chrome.storage.local.get([PREF_KEYS.theme, PREF_KEYS.locale]);
  ...
  themeButtons.forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      await chrome.storage.local.set({ [PREF_KEYS.theme]: currentTheme });
    });
  });
  ...
  localeButtons.forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      await chrome.storage.local.set({ [PREF_KEYS.locale]: currentLocale });
    });
  });
